...
Adopted 2026-07-10 (§9 Session 24), **superseding "Quiet Teal."** Direction: clean off-white paper + a single **deep-green** accent, Notion/Linear/Apple restraint — color is reserved for the ring, progress, active controls and status. **Inter** for UI/body, **Geist** (bold) for the brand/display (`src/app/layout.tsx`). Implemented as CSS custom properties in `src/app/globals.css`; light is `:root`, dark is the `.dark` class on `<html>`, managed by **next-themes** with a small fixed-position toggle cycling **light / dark / system, defaulting to system** (revised 2026-07-11, §9 Session 25 — Session 24's "no manual toggle" stance is superseded; every Tailwind utility resolves through these variables via `@theme inline`, so theme swaps still need no component changes). Also set via `@theme`: an 18px house radius (`--radius-xl`, on cards/dialogs) and an 8pt-friendly radius scale.
...
- **[Session 25]** Four changes from real use. (1) **"Remembered %" unified on the fixed 24h day (§5.8.4).** `rememberedShare` had been dividing by the narrated span (first-event-start → last-event-end), so a contiguous 17h story read "100%" while the ring said "17h" — the two now share the same denominator (and midnight-crossing events are clamped via the ring's own `clampToRange`, moved to `src/domain/timeline/gaps.ts`). Gap detection itself is untouched (§6.3's between-events rule stands). (2) **English-only UI defaults:** the Turkish `DEFAULT_CATEGORIES` seed and CategoryPicker's "+ Yeni" became English (Work, Learning, … / "+ New"); the narrator's own language is still always preserved in their data. (3) **Manual theme toggle** (light/dark/system, default system) via next-themes — supersedes Session 24's "no manual toggle"; dark moved from the OS media query to a `.dark` class so an explicit choice can win. (4) **Signed-out experience:** a one-time 4-slide onboarding intro (localStorage-gated, skippable) before the sign-in card, and **email/password sign-up/sign-in** alongside Google (same Supabase cookie sessions; `resolveDataContext` is provider-agnostic, so no data-layer change). Files: `src/domain/timeline/gaps.ts`, `src/app/api/day/handler.ts`, `src/domain/category/suggestions.ts`, `src/app/globals.css`, `src/app/layout.tsx`, `src/components/theme/*`, `src/components/auth/{LoginExperience,OnboardingIntro,EmailPasswordForm}.tsx`, `src/app/login/page.tsx`.
